...
static void show_menu() {
    ...
    printf("11. Filter by category\n");
    printf("12. Search description (KMP)\n");
    printf("13. Total in date range\n");
    printf("14. Top-K categories by spend\n");
    printf("15. Set category budget\n");
    printf("16. Show budget alerts\n");
...
int main() {
    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            if (save_user_budgets(username)) printf("Budgets saved to %s\n", BUDGETS_FILE);
        }
        else if (choice == 8) {
            if (save_user_expenses(username)) printf("Saved before exit.\n");
            save_user_income(username);
            save_user_budgets(username);
            printf("Goodbye, %s!\n", username);
            break;
        }
        else if (choice == 9) {
            ...
            int cnt = count_by_category(cat);
            printf("%d item(s) in category '%s'\n", cnt, cat);
            list_by_category(cat);
        }
        else if (choice == 12) {
            char pat[100];
            printf("Enter text to search in descriptions: ");
            read_line(pat, sizeof(pat));
            int m = search_description(pat);
            printf("%d match(es) shown.\n", m);
        }
        else if (choice == 13) {
            char from_s[20], to_s[20];
            printf("From date (yyyymmdd): "); read_line(from_s, sizeof(from_s));
            printf("To date (yyyymmdd): "); read_line(to_s, sizeof(to_s));
            int from = atoi(from_s), to = atoi(to_s);
            double tot = total_in_date_range(from, to);
            printf("Total from %d to %d: %.2f\n", from, to, tot);
        }
        else if (choice == 14) {
            char k_s[10];
            printf("Enter K: "); read_line(k_s, sizeof(k_s));
            int k = atoi(k_s);
            top_k_categories(k);
        }
        else if (choice == 15) {
            char cat[MAX_CAT], amt_s[50];
            printf("Category to budget: "); read_line(cat, sizeof(cat));
            printf("Budget amount: "); read_line(amt_s, sizeof(amt_s));
            double b = atof(amt_s);
            if (b < 0) b = 0;
            set_category_budget(cat, b);
            printf("Set budget for %s to %.2f\n", cat, b);
        }
        else if (choice == 16) {
            show_budget_alerts();
        }
